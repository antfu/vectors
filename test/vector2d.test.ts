...
describe('Vector2D', () => {
  describe('static', () => {
    it('instantiable', () => {
      expect(new Vector2D()).toBeInstanceOf(Vector2D)
    })
    it('static members', () => {
      expect(Vector2D.version).toBe(version)
      expect(Vector2D.ZERO).toBeInstanceOf(Vector2D)
    })
    it('#fromArray()', function() {
      const arr = [100, 200]
      const vec = Vector2D.fromArray(arr)

      expect(vec).toBeInstanceOf(Vector2D)
      expect(vec).toHaveProperty('x', arr[0])
      expect(vec).toHaveProperty('y', arr[1])
    })
    it('#fromObject()', function() {
      const obj1 = { x: 100, y: 200 }
      const vec1 = Vector2D.fromObject(obj1)

      expect(vec1).toBeInstanceOf(Vector2D)
      expect(vec1).toHaveProperty('x', obj1.x)
      expect(vec1).toHaveProperty('y', obj1.y)

      const obj2 = { centerX: 300, centerY: 400 }
      const vec2 = Vector2D.fromObject(obj2, 'centerX', 'centerY')
    ...
    it('#fromPolar()', function() {
      const vec = Vector2D.fromPolar(Math.PI / 3, 2)
  ...
  describe('chainable & basic operate', () => {
    ...
    it('60 degrees', () => {
      const v1 = new Vector2D(1, Math.sqrt(3))
      expect(v1.x).toBe(1)
    ...
    it('#add()', () => {
      ...
      expect(vec3).toBeInstanceOf(Vector2D)
      expect(vec4).toBeInstanceOf(Vector2D)
      expect(vec5).toBeInstanceOf(Vector2D)
    ...
    it('#rotate()', () => {
      const vec1 = new Vector2D(1, Math.sqrt(3))
